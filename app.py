...
df_cap_total = _concat("cap_total")
df_ye = _concat("ye_both")
...
# -----------------------------
# PDF Infographic Export (single page)
# -----------------------------
def _safe_float(x):
    try:
        v = float(x)
...
def _value_at(df: pd.DataFrame, scenario: str, year: int) -> float:
    if df is None or df.empty:
        return np.nan
    d = df[(df.get("scenario") == scenario) & (df.get("year") == year)]
    if d.empty:
        return np.nan
    return _safe_float(d.iloc[0].get("value"))


def _total_at(df_long: pd.DataFrame, scenario: str, year: int, group_col: str) -> float:
    if df_long is None or df_long.empty:
        return np.nan
    d = df_long[(df_long.get("scenario") == scenario) & (df_long.get("year") == year)]
    if d.empty:
        return np.nan
    return _safe_float(pd.to_numeric(d.get("value"), errors="coerce").sum())


def _top_changes(df_long: pd.DataFrame, scenario: str, y1: int, y2: int, group_col: str, topn: int = 3):
    """Return top increases and decreases between y1 and y2 for a given scenario."""
    if df_long is None or df_long.empty or group_col not in df_long.columns:
        return [], []

    d = df_long[df_long.get("scenario") == scenario].copy()
    if d.empty:
        return [], []

    d["year"] = pd.to_numeric(d["year"], errors="coerce")
    d["value"] = pd.to_numeric(d["value"], errors="coerce")
    d = d.dropna(subset=["year", "value", group_col])
    d["year"] = d["year"].astype(int)

    p = d.pivot_table(index=group_col, columns="year", values="value", aggfunc="sum")
    if (y1 not in p.columns) or (y2 not in p.columns):
        return [], []

    delta = (p[y2] - p[y1]).sort_values(ascending=False)
    inc = [(str(k), _safe_float(v)) for k, v in delta.head(topn).items() if np.isfinite(v) and abs(v) > 0]
    dec = [(str(k), _safe_float(v)) for k, v in delta.tail(topn).sort_values().items() if np.isfinite(v) and abs(v) > 0]
    return inc, dec


def _pct_change(v1: float, v2: float) -> float:
    if not np.isfinite(v1) or not np.isfinite(v2) or v1 == 0:
        return np.nan
    return (v2 - v1) / v1 * 100.0


def _pagesize(page_choice: str):
    # A4 landscape or 16:9 (approx 13.33in x 7.5in)
    if page_choice == "A4 (Yatay)":
        return landscape(A4)
    # 16:9
    return (960, 540)


def build_infographic_pdf_bytes(
    lang: str,
    page_choice: str,
    scenario_a: str,
    scenario_b: str,
    y1: int,
    y2: int,
    df_gdp_: pd.DataFrame,
    df_primary_: pd.DataFrame,
    df_supply_: pd.DataFrame,
    df_ye_: pd.DataFrame,
    df_cap_total_: pd.DataFrame,
    df_genmix_: pd.DataFrame,
    df_capmix_: pd.DataFrame,
    df_ghg_stack_: pd.DataFrame,
) -> bytes:
    if not REPORTLAB_AVAILABLE:
        raise RuntimeError("ReportLab is not available.")

    # Unicode font for TR/EN (CID)
    try:
    ...
    T = {
        "tr": {
            "title": "Ulusal Enerji Planı — Senaryo Karşılaştırma Özeti",
            "meta": "Senaryo A / B ve iki odak yıl ile otomatik üretilen tek sayfa infografik.",
            "sec_kpi": "Temel Göstergeler",
            "sec_mix": "Kaynak Bazında Değişimler",
            "gdp": "GSYH",
            "primary": "Birincil Enerji (Toplam)",
            "supply": "Elektrik Üretimi (Toplam, GWh)",
            "re_share": "Elektrikte YE Payı (%)",
            "cap": "Kurulu Güç (Toplam, GW)",
            "ghg": "Sera Gazı (Toplam, ktn CO2e)",
            "table_hdr": ["Gösterge", f"{scenario_a} ({y1})", f"{scenario_a} ({y2})", "Δ", "%Δ",
                          f"{scenario_b} ({y1})", f"{scenario_b} ({y2})", "Δ", "%Δ"],
            "inc": "En çok artanlar",
            "dec": "En çok azalanlar",
            "gen_mix": "Elektrik üretimi (kaynak, Δ)",
            "cap_mix": "Kurulu güç (kaynak, Δ)",
            "notes": "Not: Değerler, uygulamadaki aynı veri ve filtrelerle hesaplanır.",
        },
        "en": {
            "title": "National Energy Plan — Scenario Comparison Summary",
            "meta": "A one‑page infographic generated from Scenario A/B and two focus years.",
            "sec_kpi": "Key Indicators",
            "sec_mix": "Source-based Changes",
            "gdp": "GDP",
            "primary": "Primary Energy (Total)",
            "supply": "Electricity Generation (Total, GWh)",
            "re_share": "RES Share in Electricity (%)",
            "cap": "Installed Capacity (Total, GW)",
            "ghg": "GHG (Total, kt CO2e)",
            "table_hdr": ["Indicator", f"{scenario_a} ({y1})", f"{scenario_a} ({y2})", "Δ", "%Δ",
                          f"{scenario_b} ({y1})", f"{scenario_b} ({y2})", "Δ", "%Δ"],
            "inc": "Top increases",
            "dec": "Top decreases",
            "gen_mix": "Electricity generation mix (Δ)",
            "cap_mix": "Capacity mix (Δ)",
            "notes": "Note: Values are computed from the same data and filters as the app.",
        },
    }[lang]

    # --- KPI values ---
    gdp_a1, gdp_a2 = _value_at(df_gdp_, scenario_a, y1), _value_at(df_gdp_, scenario_a, y2)
    gdp_b1, gdp_b2 = _value_at(df_gdp_, scenario_b, y1), _value_at(df_gdp_, scenario_b, y2)

    primary_a1, primary_a2 = _total_at(df_primary_, scenario_a, y1, "source"), _total_at(df_primary_, scenario_a, y2, "source")
    primary_b1, primary_b2 = _total_at(df_primary_, scenario_b, y1, "source"), _total_at(df_primary_, scenario_b, y2, "source")

    supply_a1, supply_a2 = _value_at(df_supply_, scenario_a, y1), _value_at(df_supply_, scenario_a, y2)
    supply_b1, supply_b2 = _value_at(df_supply_, scenario_b, y1), _value_at(df_supply_, scenario_b, y2)

    # RES share in electricity: from df_ye_ (series includes "Toplam YE")
    re_a1 = np.nan
    re_a2 = np.nan
    re_b1 = np.nan
    re_b2 = np.nan
    if df_ye_ is not None and not df_ye_.empty:
        sub = df_ye_[df_ye_.get("series").astype(str).str.contains("Toplam", na=False)]
        re_a1 = _value_at(sub, scenario_a, y1)
        re_a2 = _value_at(sub, scenario_a, y2)
        re_b1 = _value_at(sub, scenario_b, y1)
        re_b2 = _value_at(sub, scenario_b, y2)

    cap_a1, cap_a2 = _value_at(df_cap_total_, scenario_a, y1), _value_at(df_cap_total_, scenario_a, y2)
    cap_b1, cap_b2 = _value_at(df_cap_total_, scenario_b, y1), _value_at(df_cap_total_, scenario_b, y2)

    # GHG total: if stack provided, sum categories
    ghg_a1, ghg_a2 = _total_at(df_ghg_stack_, scenario_a, y1, "category"), _total_at(df_ghg_stack_, scenario_a, y2, "category")
    ghg_b1, ghg_b2 = _total_at(df_ghg_stack_, scenario_b, y1, "category"), _total_at(df_ghg_stack_, scenario_b, y2, "category")
    if not np.isfinite(ghg_a1) or not np.isfinite(ghg_a2) or not np.isfinite(ghg_b1) or not np.isfinite(ghg_b2):
        # fallback to CO2 series if stack is missing
        pass

    kpi_rows = [
        [T["gdp"], gdp_a1, gdp_a2, gdp_a2 - gdp_a1, _pct_change(gdp_a1, gdp_a2), gdp_b1, gdp_b2, gdp_b2 - gdp_b1, _pct_change(gdp_b1, gdp_b2)],
        [T["primary"], primary_a1, primary_a2, primary_a2 - primary_a1, _pct_change(primary_a1, primary_a2), primary_b1, primary_b2, primary_b2 - primary_b1, _pct_change(primary_b1, primary_b2)],
        [T["supply"], supply_a1, supply_a2, supply_a2 - supply_a1, _pct_change(supply_a1, supply_a2), supply_b1, supply_b2, supply_b2 - supply_b1, _pct_change(supply_b1, supply_b2)],
        [T["re_share"], re_a1, re_a2, re_a2 - re_a1, _pct_change(re_a1, re_a2), re_b1, re_b2, re_b2 - re_b1, _pct_change(re_b1, re_b2)],
        [T["cap"], cap_a1, cap_a2, cap_a2 - cap_a1, _pct_change(cap_a1, cap_a2), cap_b1, cap_b2, cap_b2 - cap_b1, _pct_change(cap_b1, cap_b2)],
        [T["ghg"], ghg_a1, ghg_a2, ghg_a2 - ghg_a1, _pct_change(ghg_a1, ghg_a2), ghg_b1, ghg_b2, ghg_b2 - ghg_b1, _pct_change(ghg_b1, ghg_b2)],
    ]

    # Source changes
    inc_gen_a, dec_gen_a = _top_changes(df_genmix_, scenario_a, y1, y2, "group", topn=3)
    inc_gen_b, dec_gen_b = _top_changes(df_genmix_, scenario_b, y1, y2, "group", topn=3)

    inc_cap_a, dec_cap_a = _top_changes(df_capmix_, scenario_a, y1, y2, "group", topn=3)
    inc_cap_b, dec_cap_b = _top_changes(df_capmix_, scenario_b, y1, y2, "group", topn=3)
    ...
    doc = SimpleDocTemplate(
        buff,
        pagesize=_pagesize(page_choice),
        leftMargin=28,
        rightMargin=28,
        topMargin=24,
        bottomMargin=20,
    ...
    h1 = ParagraphStyle("h1", parent=styles["Title"], fontName=base_font, fontSize=15, leading=18, spaceAfter=8)
    h2 = ParagraphStyle("h2", parent=styles["Heading2"], fontName=base_font, fontSize=11, leading=14, spaceAfter=6)
    normal = ParagraphStyle("normal", parent=styles["Normal"], fontName=base_font, fontSize=9, leading=11)

    story = []
    story.append(Paragraph(T["title"], h1))
    story.append(Paragraph(f"<b>{scenario_a}</b> vs <b>{scenario_b}</b> — {y1} → {y2}", normal))
    story.append(Paragraph(T["meta"], normal))
    story.append(Spacer(1, 10))

    story.append(Paragraph(T["sec_kpi"], h2))

    # Table
    table_data = [T["table_hdr"]]
    for r in kpi_rows:
        table_data.append([
            r[0],
            _fmt(r[1]), _fmt(r[2]), _fmt(r[3]), _fmt(r[4], is_pct=True),
            _fmt(r[5]), _fmt(r[6]), _fmt(r[7]), _fmt(r[8], is_pct=True),
        ])
    ...
    story.append(Paragraph(T["sec_mix"], h2))

    def _mix_block(title, inc, dec):
        lines = []
        if inc:
            inc_txt = ", ".join([f"{k} ({_fmt(v)})" for k, v in inc])
            lines.append(f"<b>{T['inc']}:</b> {inc_txt}")
        if dec:
            dec_txt = ", ".join([f"{k} ({_fmt(v)})" for k, v in dec])
            lines.append(f"<b>{T['dec']}:</b> {dec_txt}")
        if not lines:
            lines = ["—"]
        return Paragraph(f"<b>{title}</b><br/>" + "<br/>".join(lines), normal)

    story.append(_mix_block(f"{T['gen_mix']} — {scenario_a}", inc_gen_a, dec_gen_a))
    story.append(Spacer(1, 4))
    story.append(_mix_block(f"{T['gen_mix']} — {scenario_b}", inc_gen_b, dec_gen_b))
    story.append(Spacer(1, 6))
    story.append(_mix_block(f"{T['cap_mix']} — {scenario_a}", inc_cap_a, dec_cap_a))
    story.append(Spacer(1, 4))
    story.append(_mix_block(f"{T['cap_mix']} — {scenario_b}", inc_cap_b, dec_cap_b))
    story.append(Spacer(1, 8))

    story.append(Paragraph(T["notes"], normal))
