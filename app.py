...
# Electricity supply (GWh) – your reference row
...
# Installed capacity: exclude subtotals / group headers (do not count)
...
# For CAPACITY MIX:
# - "Other Renewables" should be included inside "Other"
# - Battery + Pumped Storage + Demand Response should be shown as "Storage" (single bucket)
# - Residual "Other" should be computed to match TOTAL_CAPACITY_LABEL each year
CAPACITY_STORAGE_REGEX = re.compile(
    r"(pumped\s+storage|\bbattery\b|\bstorage\b|demand\s+response)",
    flags=re.IGNORECASE,
)

# Technology mapping (generation + capacity)
TECH_GROUPS = {
    "Hydro": [r"\bhydro\b"],
    ...
    # keep for generation RES share calc, but we will fold it into "Other" for capacity mix
...
def total_capacity_series(installed_cap_df: pd.DataFrame) -> pd.DataFrame:
    """
    Toplam kurulu güç (GW): doğrudan TOTAL_CAPACITY_LABEL satırından.
...
def capacity_mix_from_block(installed_cap_df: pd.DataFrame, cap_total: pd.DataFrame) -> pd.DataFrame:
    ...
    - Ara toplamları hariç tutar
    - Toplamı her yıl TOTAL_CAPACITY_LABEL satırından referans alır
    - "Storage" = (Battery + Pumped Storage + Demand Response)
    - "Other Renewables" -> "Other" içinde gösterilir
    - "Other" = Total - (bilinen gruplar + Storage) residual
    """
    ...
    df = installed_cap_df.copy()
    df = df[~df["item"].apply(_capacity_is_excluded)]
    df = df[df["item"].str.strip().ne(TOTAL_CAPACITY_LABEL)]  # toplam satırı karmaya girmesin

    long = _to_long(df, value_name="value")

    # Storage bucket
    long["is_storage"] = long["item"].astype(str).apply(lambda s: bool(CAPACITY_STORAGE_REGEX.search(s)))
    storage = long[long["is_storage"]].groupby("year", as_index=False)["value"].sum()
    storage["group"] = "Storage"

    # Remaining items (excluding storage items)
    rest = long[~long["is_storage"]].copy()
    rest["group"] = rest["item"].apply(_strict_match_group)

    # Fold "Other Renewables" into "Other" (capacity only)
    rest.loc[rest["group"] == "Other Renewables", "group"] = "Other"

    mix = rest.groupby(["year", "group"], as_index=False)["value"].sum()
    mix = mix[mix["year"] <= MAX_YEAR]

    # merge storage (if any)
    if not storage.empty:
        storage = storage[storage["year"] <= MAX_YEAR]
        mix = pd.concat([mix, storage[["year", "group", "value"]]], ignore_index=True)

    # --- Residual Other: total - sum(ALL groups except Other)
    ...
    known = (
        ...
        .sum()
        .rename(columns={"value": "known_sum"})
    )
    ...
    # Safety: any year missing in mix -> all as Other
...
# Installed capacity (GW) – total from TOTAL_CAPACITY_LABEL
...
if gen_mix.empty:
    ...
    order_gen = ["Hydro", "Wind (RES)", "Solar (GES)", "Other Renewables", "Gas", "Coal", "Lignite", "Nuclear", "Other"]
...
st.caption("Not: Other Renewables, Other içinde; Battery + Pumped Storage + Demand Response = Storage; Other residual (Toplam satıra eşitlenir).")

if cap_mix.empty:
    st.warning("Kurulu güç karması çıkarılamadı.")
else:
    order_cap = ["Hydro", "Wind (RES)", "Solar (GES)", "Gas", "Coal", "Lignite", "Nuclear", "Storage", "Other"]
