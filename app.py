...
if run_btn:
    ...
    # -------------------------------------------------
    # Opsiyon: Tüm yıllar için profiles parquet üret
    # -------------------------------------------------
    produce_all_years = st.checkbox(
        "Tüm yıllar için (2021–2025) profiles_YYYY.parquet üret",
        value=False,
        help="İşaretlersen, mevcut ham veride bulunan tüm yıllar için 8760 profil parquet dosyaları üretilir."
    )
    ...
    with st.spinner("Parquet yazılıyor..."):
        ...
        def _write_profiles_for_year(_yr: int, _df_year: pd.DataFrame):
            _prof = pd.DataFrame(index=_df_year.index)
            _prof["load_base"] = _df_year["consumption_mwh"]
            _prof["net_load_base"] = _df_year["net_load_mwh"]
            _prof["solar_shape"] = normalize_shape_max(_df_year["solar_total_mwh"])
            _prof["wind_shape"]  = normalize_shape_max(_df_year["wind_total_mwh"])
            _prof["hydro_res_shape"] = normalize_shape_max(_df_year["hydro_res_mwh"])
            _prof["hydro_ror_shape"] = normalize_shape_max(_df_year["hydro_ror_mwh"])
            _prof["hydro_shape"] = normalize_shape_max(_df_year["hydro_mwh"])
            _path = OUT_DIR / f"profiles_{_yr}.parquet"
            _prof.reset_index().rename(columns={"index": "timestamp"}).to_parquet(_path, index=False)
            return _path, _prof

        if produce_all_years:
            # Seçili baz yıl profili zaten üretildi: prof_out
            ...
            written_profiles.append(profiles_path)

            # Diğer yılları üret
            for yr in years_available:
                if int(yr) == int(base_year):
                    continue
                df_y = out[out.index.year == yr].copy()
                if len(df_y) != 8760:
                    st.warning(f"{yr} yılı için satır sayısı 8760 değil: {len(df_y)} (atlandı).")
                    continue
                pth, _ = _write_profiles_for_year(int(yr), df_y)
                written_profiles.append(pth)

        else:
            # Sadece seçili baz yıl
            profiles_path = OUT_DIR / f"profiles_{base_year}.parquet"
            prof_out.reset_index().rename(columns={"index": "timestamp"}).to_parquet(profiles_path, index=False)
            written_profiles.append(profiles_path)
    ...
    if produce_all_years:
        with st.expander("Diğer yılların profiles dosyaları (indir)", expanded=False):
            for _p in written_profiles:
